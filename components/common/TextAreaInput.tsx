...
const TextAreaInput: React.FC<TextAreaInputProps> = ({ label, id, value, ...props }) => {
  ...
  return (
    <div className="relative bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-md focus-within:border-zinc-400 dark:focus-within:border-zinc-500">
      ...
      <textarea
        ...
        value={value}
        {...props}
        className={`w-full bg-transparent p-2 text-xs rounded-md dark:text-zinc-100 min-h-[80px] resize-none
          ${hasValue ? 'pt-5 pb-1' : ''}
          focus:outline-none`}
      />
    </div>
